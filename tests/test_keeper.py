...
from keeper.analyzer import SAVE_TOOL, AnalysisError, ScreenshotAnalyzer, prepare_image
from keeper.config import Settings
from keeper.db import Database, fts_query
from keeper.enrich import Page, enrich_screen, iso_duration, recipe_from_page, run_enrichers
from keeper.main import create_app
from keeper.pipeline import merge
...
def test_media_path_traversal_blocked(settings):
    client, _ = make_client(settings, analysis())
    with client:
        assert client.get("/media/..%2Fkeeper.db").status_code == 404
