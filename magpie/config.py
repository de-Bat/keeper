...
@dataclass
class Settings:
    data_dir: Path = field(default_factory=lambda: Path(os.environ.get("MAGPIE_DATA_DIR", "data")).resolve())
    model: str = field(default_factory=lambda: os.environ.get("MAGPIE_MODEL", "claude-opus-5"))
    ...
    api_token: str | None = field(default_factory=lambda: os.environ.get("MAGPIE_API_TOKEN") or None)

    # Which analyzer identifies screenshots: auto | claude | local | hybrid | ocr
    analyzer: str = field(default_factory=lambda: _env("MAGPIE_ANALYZER", "auto").lower())
    ...
    escalate_below: int = field(default_factory=lambda: int(_env("MAGPIE_ESCALATE_BELOW", "70")))
    # OCR pre-pass: rapidocr (bundled, CPU) | tesseract (needs the binary; better for Hebrew/Arabic/...) | off
    ocr_engine: str = field(default_factory=lambda: _env("MAGPIE_OCR", "rapidocr").lower())
    ocr_langs: str = field(default_factory=lambda: _env("MAGPIE_OCR_LANGS", "eng"))  # tesseract only, e.g. eng+heb
    # Cost controls for Claude (see docs/COSTS.md)
    effort: str = field(default_factory=lambda: _env("MAGPIE_EFFORT", "medium").lower())         # low|medium|high|xhigh|max
    fetch_max_tokens: int = field(default_factory=lambda: int(_env("MAGPIE_FETCH_MAX_TOKENS", "8000")))  # 0 = no cap
    # Send new screenshots to Claude through the Message Batches API (50% cheaper; results in minutes, max 24 h)
    claude_batch: bool = field(default_factory=lambda: _env("MAGPIE_CLAUDE_BATCH", "true").lower() not in ("0", "false", "no", "off"))
    batch_poll_seconds: int = field(default_factory=lambda: int(_env("MAGPIE_BATCH_POLL_SECONDS", "60")))
    # Running cost of your on-prem inference box, for the usage report (e.g. 350 W at $0.20/kWh = 0.07)
    local_cost_per_hour: float = field(default_factory=lambda: float(_env("MAGPIE_LOCAL_COST_PER_HOUR", "0")))

    # Online metadata lookups (TMDB, GitHub, recipe pages...). Turn off for air-gapped installs.
    enrich: bool = field(default_factory=lambda: _env("MAGPIE_ENRICH", "on").lower() not in ("0", "off", "false", "no"))
