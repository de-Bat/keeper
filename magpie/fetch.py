...
Magpie fetches URLs that come from outside: links you share, and URLs a model read off a
...
address. Set MAGPIE_ALLOW_PRIVATE_URLS=true to allow LAN addresses on purpose.
"""
...
def _allow_private() -> bool:
    return os.environ.get("MAGPIE_ALLOW_PRIVATE_URLS", "").lower() in ("1", "true", "yes", "on")
...
async def check_url(url: str) -> None:
    ...
    for addr in addresses:
        ...
        if not ip.is_global:
            raise BlockedURL(f"{host} resolves to a non-public address ({ip}); set MAGPIE_ALLOW_PRIVATE_URLS=true to allow")
